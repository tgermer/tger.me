...
# Build MD frontmatter (always references the snapshot)
{
    ...
    echo ""
    echo "# === Attachments ==="
    echo "# Dokument-IDs aus src/data/documents.ts (z.B. az-hm-2024, masterzeugnis-hm-2020)"
    echo "documents: []"
    echo ""
    echo "# === Intern ==="
    echo "token: \"${TOKEN}\""
    ...
    echo ""
    echo "# === Status ==="
    echo "# Werte: gespeichert | beworben | eingangsbestätigung | einladung | vorstellungsgespräch | zweitgespräch | assessment | angebot | zusage | absage | zurückgezogen"
