...
public/apply/<slug>-anlagen.pdf ← Anlagen PDF (merged attachments, only if documents set)
```
...
│   ├── documents.ts           ← Document attachment registry (Anlagen)
...
└── apply/
    ...
    └── <slug>-anlagen.pdf     ← Merged Anlagen PDFs (committed to git, only if documents set)
.github/
└── workflows/
...
- **Anlagen PDF** (`<slug>-anlagen.pdf`) — merged PDF of all selected documents, generated by `npm run pdf`
- **UI**: Anlagen section on `[slug].astro` (screen only) with individual PDF links + toolbar download button
...
4. Run `npm run pdf <slug>` → generates `<slug>-anlagen.pdf`
5. Commit & push (both the source PDF in `docs/` and the generated `-anlagen.pdf`)
